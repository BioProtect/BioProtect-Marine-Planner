...
def _setGlobalVariables():
    ...
    CASE_STUDY_PROJECT_FOLDER = MARXAN_WEB_RESOURCES_FOLDER + \
        "British Columbia Marine Case Study" + os.sep
...
async def _getProjectData(obj):
    ...
    for k in keys:
        ...
        elif k in ['DESCRIPTION', 'CREATEDATE', 'PLANNING_UNIT_NAME', 'OLDVERSION', 'IUCN_CATEGORY', 'PRIVATE']:
            key, value = _getKeyValue(s, k)
...
def _invalidateProtectedAreaIntersections():
    ...
    files = _getFilesInFolderRecursive(
        MARXAN_FOLDER, PROTECTED_AREA_INTERSECTIONS_FILENAME)
...
def _addParameter(_type, key, value):
    ...
    if (_type == 'user'):
        ...
        _files = glob.glob(MARXAN_USERS_FOLDER + os.sep +
                           "*" + os.sep + "user.dat")
    else:
...
def _deleteZippedShapefile(folder, zipfile, archivename):
    ...
    if len(files) > 0:
        [os.remove(f) for f in files if f[-3:] in ['shx', 'shp', 'xml', 'sbx', 'prj', 'sbn', 'zip',
                                                   'dbf', 'cpg', 'qpj', 'SHX', 'SHP', 'XML', 'SBX', 'PRJ', 'SBN', 'ZIP', 'DBF', 'CPG', 'QPJ']]
...
async def _uploadTilesetToMapbox(feature_class_name, mapbox_layer_name):
    ...
    cmd = '"' + OGR2OGR_EXECUTABLE + '" -f "ESRI Shapefile" "' + MARXAN_FOLDER + feature_class_name + '.shp"' + ' "PG:host=' + DATABASE_HOST + ' dbname=' + DATABASE_NAME + \
        ' user=' + DATABASE_USER + ' password=' + DATABASE_PASSWORD + '" -sql "select * from Marxan.' + \
        feature_class_name + '" -nln ' + mapbox_layer_name + ' -t_srs EPSG:3857'
    logging.debug(cmd)
    try:
        # run the export
        if platform.system() != "Windows":
            # asynchronous on unix
    ...
    _deleteZippedShapefile(
        MARXAN_FOLDER, feature_class_name + ".zip", feature_class_name)
    return uploadId
...
class PostGIS():
    ...
    async def execute(self, sql, data=None, format="Array", filename=None):
        with (await self.pool.cursor()) as cur:
            ...
            try:
                ...
                raise MarxanServicesError(
                    "Database integrity error: " + e.args[0])
            except psycopg2.OperationalError as e:
                if ("terminating connection due to administrator command" in e.args[0]):
                    raise MarxanServicesError(
                        "The database server was shutdown")
            except Exception as e:
                raise MarxanServicesError(e.args[0])
    ...
    async def importShapefile(self, shapefile, feature_class_name, epsgCode, splitAtDateline=True):
        try:
            ...
            cmd = '"' + \
                OGR2OGR_EXECUTABLE + '" -f "PostgreSQL" PG:"host=' + \
                DATABASE_HOST + ' user=' + \
                DATABASE_USER + ' dbname=' + \
                DATABASE_NAME + ' password=' + \
                DATABASE_PASSWORD + '" "' + \
                MARXAN_FOLDER + shapefile + \
                '" -nlt GEOMETRY -lco SCHEMA=marxan -lco GEOMETRY_NAME=geometry -nln ' + \
                feature_class_name + ' -t_srs ' + epsgCode + ' -lco precision=NO'
    ...
    async def isValid(self, feature_class_name):
        ...
        _isValid = await self.query(sql.SQL("SELECT DISTINCT ST_IsValid(geometry) FROM marxan.{} LIMIT 1;").format(sql.Identifier(feature_class_name)))
        if not _isValid[0][0]:
            # delete the feature class
            await self.execute(sql.SQL("DROP TABLE IF EXISTS marxan.{};").format(sql.Identifier(feature_class_name)))
...
class methodNotFound(MarxanRESTHandler):
    def prepare(self):
        ...
        raise tornado.web.HTTPError(
            501, "The method is not supported or the parameters are wrong on this Marxan Server " + MARXAN_SERVER_VERSION)
...
class createUser(MarxanRESTHandler):
    def post(self):
        ...
        _cloneProject(START_PROJECT_FOLDER, MARXAN_USERS_FOLDER +
                      self.get_argument('user') + os.sep)
        # copy the british columbia marine case study into the users folder
        _cloneProject(CASE_STUDY_PROJECT_FOLDER,
                      MARXAN_USERS_FOLDER + self.get_argument('user') + os.sep)
        # set the response
        self.send_response(
...
class createImportProject(MarxanRESTHandler):
    def post(self):
        ...
        self.send_response({'info': "Project '" + self.get_argument('project') +
                            "' created", 'name': self.get_argument('project')})
    ...
    async def get(self):
        ...
        if (self.get_argument("user") == GUEST_USERNAME):
            ...
            self.send_response({'user': self.get_argument("user"), 'project': self.projectData["project"], 'metadata': self.projectData["metadata"], 'files': self.projectData["files"], 'runParameters': self.projectData["runParameters"], 'renderer': self.projectData["renderer"], 'features': self.speciesData.to_dict(
                orient="records"), 'feature_preprocessing': self.speciesPreProcessingData.to_dict(orient="split")["data"], 'planning_units': self.planningUnitsData, 'protected_area_intersections': self.protectedAreaIntersectionsData})
...
class uploadShapefile(MarxanRESTHandler):
    def post(self):
        ...
        _writeFile(MARXAN_FOLDER + self.get_argument('filename'),
...
class getShapefileFieldnames(MarxanRESTHandler):
    def get(self):
        ...
        fields = _getShapefileFieldNames(
            MARXAN_FOLDER + self.get_argument('filename'))
        # set the response
        self.send_response(
...
class deleteShapefile(MarxanRESTHandler):
    def get(self):
        ...
        _deleteZippedShapefile(MARXAN_FOLDER, self.get_argument(
                    ...
                    try:
                        if platform.system() != "Windows":
                            ...
                            self.marxanProcess = Subprocess(
                                ["exec " + MARXAN_EXECUTABLE], stdout=Subprocess.STREAM, stdin=PIPE, shell=True)
    ...
    async def stream_marxan_output(self):
        if platform.system() != "Windows":
            try:
                ...
                log("The WebSocket is already closed. Unable to stream Marxan output for pid = " +
            ...
            try:
                ...
                # error unzipping - either the polygons shapefile does not exist or the disk space has run out
                    ...
                    try:
                        ...
                        _deleteZippedShapefile(
                            MARXAN_FOLDER, WDPA_DOWNLOAD_FILE, rootfilename)
...
if __name__ == "__main__":
    try:
        ...
        root_streamhandler.setFormatter(LogFormatter(
            fmt='%(color)s[%(levelname)1.1s %(asctime)s.%(msecs)03d]%(end_color)s %(message)s', datefmt='%d-%m-%y %H:%M:%S', color=True))
        # add a file logger
        if not DISABLE_FILE_LOGGING:
            file_log_handler = logging.FileHandler(
                MARXAN_FOLDER + MARXAN_LOG_FILE)
            file_log_handler.setFormatter(LogFormatter(
                fmt='%(color)s[%(levelname)1.1s %(asctime)s.%(msecs)03d]%(end_color)s %(message)s', datefmt='%d-%m-%y %H:%M:%S', color=False))
        ...
        try:
            ...
            log("Shutting down marxan-server at " +

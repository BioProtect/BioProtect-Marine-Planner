...
# Create a Marxan Grid and required Marxan ".dat" files 

# Specify if you want a square or a hexagonal grid:
GridShape <- "hexagonal" #must be either "square" or "hexagonal"
CellArea <- 10000*10000 #area of Planning Units in m2 

PULayer <- make_grid(Extents, type = GridShape, cell_area = CellArea, clip = T)

PUnames <- as.data.frame(1:ncell(PULayer))
colnames(PUnames)[1] <- "puid"
PULayer <- SpatialPolygonsDataFrame(PULayer, PUnames)
rm(PUnames)